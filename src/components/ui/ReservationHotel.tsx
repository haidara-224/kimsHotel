...
import { getUserReservationsWithHotel, getUserReservationsWithLogement } from "@/app/(action)/reservation.action";
...
export default function ReservationHotel() {
    ...
    const [reservationsL, setReservationl] = useState<Reservation[]>([]);
    ...
    useEffect(() => {
     ...
     const reservationLogement = async () => {
        try {
            const data = await getUserReservationsWithLogement();
            setReservationl(data as unknown as Reservation[]);
        } catch (error) {
            console.error("Error fetching reservations:", error);
        } finally {
            setLoading(false);
                    ...
                    {reservationsL.map((reservation) => (
                        <Card key={reservation.id} className="overflow-hidden transition-all hover:shadow-md">
                            ...
                            <CardHeader className="pb-3">
                                ...
                                <div className="text-sm text-gray-500">
                                    Adresse #{reservation.logement.telephone}
                                </div>
                            </CardHeader>
                            <CardContent className="space-y-3">
                                <div className="flex items-start space-x-3">
