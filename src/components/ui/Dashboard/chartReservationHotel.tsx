...
import { RepportReservation } from "@/app/(action)/hotel.action";
...
export function ChartReservation({ hotelId }: { hotelId: string }) {
  const [chartData, setChartData] = useState<ChartData[]>([]);

  useEffect(() => {
    async function fetchData() {
      const data = await RepportReservation(hotelId);
      if (data) {
